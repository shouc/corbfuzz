...
void CFAskField(zval* key_zv, zval* seed_zv, zval* res, int seedi){
    char* key = Z_STRVAL_P(key_zv);
    int key_len = Z_STRLEN_P(key_zv);
    char* seed = Z_STRVAL_P(seed_zv);
    int seed_len = Z_STRLEN_P(seed_zv);
    ...
    char* send_str = malloc(key_len + seed_len + 4);
    strcpy(send_str, seed);
    strcat(send_str, "%%");
    char seedi_s[255];
    sprintf(seedi_s, "%d", seedi);
    strcat(send_str, seedi_s);
    strcat(send_str, "%%");
    strcat(send_str, key);
    strcat(send_str, "%%0");

    send(server_, send_str, key_len + seed_len + 7 + strlen(seedi_s), 0);

    char buffer[1024] = {0};
    int valread = read(server_ , buffer, 1024);
    int field_cache = 0;
    int field_cache_cter = 0;
    ...
    for (int i = valread - 1; i >= 0; --i) {
        ...
        if (step == 0){
            field_cache += (buffer[i] - '0') * (pow(10, field_cache_cter++));
        }
        if (step == 1){
            type_c = buffer[i] - '0';
        }
    ...
    switch (type_c) {
        case 0:
            // string
            ZVAL_STRING(res, buffer);
            break;
        case 1:
            // int
            ZVAL_LONG(res, atoi(buffer));
            break;
        case 2:
            // double
            ZVAL_DOUBLE(res, strtod(buffer, NULL));
            break;
        case 3:
            // bool
            ZVAL_BOOL(res, buffer[0] == 't');
            break;
		default:
            ZVAL_LONG(res, 1);
    }
    res->u1.v.u.extra = 6000 + field_cache + 80 * seedi;
...
#define corbfuzz_ht ht
#define corb_fuzz_hav corbfuzz_ht->arData->val
#define cobrfuzz_h_seedi corbfuzz_ht->arData[1].val
static zend_always_inline zval *zend_fetch_dimension_address_inner2(HashTable *ht, zval* container, const zval *dim, int dim_type, int type EXECUTE_DATA_DC)
{

    unsigned int seedii = 1;
    char is_cbfuzz = 0;
    int seedi = 0;
    if (corbfuzz_ht->arData && Z_TYPE(corb_fuzz_hav) == IS_STRING) {
        if (strcmp(Z_STRVAL(corb_fuzz_hav), "1333333337") == 0){
            assert(Z_TYPE(cobrfuzz_h_seedi) == IS_STRING);
prev_seedi:
            seedi = atoi(Z_STRVAL(ht->arData[seedii].val));
            if (seedi == CORBFUZZ_MAGIC_NUM){
                seedii++;
                goto prev_seedi;
            }
            is_cbfuzz = 1;
	...
	if (container->u1.v.u.extra >= 8 && container->u1.v.u.extra < 6000) {
		zval* res = malloc(sizeof(zval));
		printf("Handling Session/Cookie\n");
		ZVAL_LONG(res, 1);
		res->u1.v.u.extra = 8;
        return res;
    }
    ...
    if (is_cbfuzz != 1){
    	zval* res = zend_fetch_dimension_address_inner(Z_ARRVAL_P(container), dim, IS_CONST, BP_VAR_R EXECUTE_DATA_CC);
        return res;
    }
	zval* res = malloc(sizeof(zval));
	...
	zval* seed = zend_hash_str_find(Z_ARRVAL_P(carrier),
									"HTTP_SEED",
									sizeof("HTTP_SEED") - 1);
	CFAskField(dim, seed, res, seedi);
	printf("Handling CorbFuzz Query %u\n", res->u1.v.u.extra);
    return res;
}
