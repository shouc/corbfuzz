...
MYSQLND_METHOD(mysqlnd_command, query)(MYSQLND_CONN_DATA * const conn, MYSQLND_CSTRING query)
{
    ...
    seed_len = Z_STRLEN_P(seed) + 1;
    strncat(query.s, ";", 1);
    strncat(query.s, seed_s, seed_len - 1);

work_now:
	ret = send_command(conn->payload_decoder_factory, COM_QUERY, (const zend_uchar*) query.s, query.l + seed_len, FALSE,
					   &conn->state,
					   conn->error_info,
					   conn->upsert_status,
					   conn->stats,
